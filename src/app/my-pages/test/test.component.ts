...
import { FullCalendarComponent, CalendarOptions, Calendar, EventInput, EventApi, EventSourceInput } from '@fullcalendar/angular'; // useful for typechecking
        ...
        if (result) {  // in case of cancel the result will be false
          let sub = this.agendaService.create$(result)
            .subscribe(addResult => {
              ...
              this.calendarComponent.getApi().addEvent(this.agendaToEvent(toBeAdded));
...
/***************************************************************************************************/
  handleEventClick(arg) {
    ...
    // Een deep copy
    let toBeEdited: AgendaItem = JSON.parse(JSON.stringify(arg.event.extendedProps));
        ...
        if (result) {  // in case of cancel the result will be false
            let sub = this.agendaService.update$(result)
              ...
              // this.calendarComponent.getApi().
                  // arg.event.setDates(result.Datum, null, true);
                  arg.event.setStart(result.Datum);
                  arg.event.setEnd(null);
                  arg.event.setProp('title', result.EvenementNaam);
                  arg.event.setProp('ExtendedProps', result);

                    //hier de angemda aanpassem
  ...
  calendarOptions: CalendarOptions = {
    ...
    headerToolbar: { start: 'title', center: 'today prev,next', end: 'dayGridWeek,dayGridMonth,listMonth' },
